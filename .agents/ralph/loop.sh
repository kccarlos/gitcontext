...
with prd_path.open("r+", encoding="utf-8") as fh:
    ...
    try:
        ...
        if isinstance(defaults, dict):
            max_attempts = parse_attempts(defaults.get("maxAttemptsPerStory"))
            if max_attempts <= 0:
                max_attempts = 5
        ...
        for story in stories:
            ...
            passes = to_bool(story.get("passes"))
            if status == "done" and not passes:
                integrity_errors.append(str(story.get("id", "(unknown)")))
                ...
                json.dumps(
                    {
                        "ok": False,
                        "error": (
                            "PRD integrity violation: status=done but passes=false for "
        ...
        def is_passed(story_id: str) -> bool:
            target = story_index.get(story_id)
            if not isinstance(target, dict):
                return False
            return to_bool(target.get("passes"))
        ...
        for story in stories:
            ...
            if not isinstance(deps, list):
                deps = []
            if all(is_passed(dep) for dep in deps):
                candidate = story
                break

        remaining = sum(
            1 for story in stories
            if isinstance(story, dict) and not to_bool(story.get("passes"))
        )
        ...
        if candidate:
            attempts = parse_attempts(candidate.get("attempts")) + 1
            ...
            if not isinstance(verification, dict):
                verification = {}
            pass_criteria = verification.get("passCriteria")
            if not isinstance(pass_criteria, list):
                pass_criteria = candidate.get("passCriteria")
            if not isinstance(pass_criteria, list):
                pass_criteria = candidate.get("acceptanceCriteria")
            if not isinstance(pass_criteria, list):
                pass_criteria = []
            pass_criteria = [str(item) for item in pass_criteria if str(item).strip()]

            verify_artifacts = verification.get("artifacts")
            if not isinstance(verify_artifacts, list):
                verify_artifacts = []
            verify_artifacts = [str(item) for item in verify_artifacts if str(item).strip()]
            ...
            block_lines.append(f"Attempt: {attempts} of {max_attempts}")
            block_lines.append("")
...
remaining = sum(
    1 for story in stories
    if isinstance(story, dict) and story.get("passes") is not True
)
print(remaining)
PY
}
...
finalize_story() {
  ...
  local note="$3"
  local result_out="$4"
  python3 - "$PRD_PATH" "$story_id" "$outcome" "$note" "$result_out" <<'PY'
import json
import os
import sys
from pathlib import Path
...
outcome = sys.argv[3]
note = sys.argv[4]
result_out = Path(sys.argv[5]) if len(sys.argv) > 5 and sys.argv[5] else None
...
def max_attempts_from(data):
    defaults = data.get("defaults") if isinstance(data, dict) else {}
    value = 5
    if isinstance(defaults, dict):
        value = parse_attempts(defaults.get("maxAttemptsPerStory"))
    return value if value > 0 else 5

def merge_note(existing, new_note):
...
with prd_path.open("r+", encoding="utf-8") as fh:
    ...
    try:
        ...
        stories = data.get("stories") if isinstance(data, dict) else None
        if not isinstance(stories, list):
            raise RuntimeError("PRD stories are missing")

        max_attempts = max_attempts_from(data)
        ...
        now = now_iso()
        attempts = parse_attempts(found.get("attempts"))
        skipped = False
        ...
        if outcome == "pass":
            ...
            found["passes"] = True
            found["skipped"] = False
            if not found.get("startedAt"):
                found["startedAt"] = now
            found["completedAt"] = now
            found["updatedAt"] = now
            found["notes"] = merge_note(found.get("notes"), note)
        else:
            found["passes"] = False
            found["updatedAt"] = now
            if attempts >= max_attempts:
                ...
                if not found.get("startedAt"):
                    found["startedAt"] = now
                found["completedAt"] = now
                cb_note = f"CIRCUIT_BREAKER: reached max attempts ({max_attempts})."
                if note:
                    cb_note = cb_note + " " + note
                found["notes"] = merge_note(found.get("notes"), cb_note)
            else:
                found["status"] = "open"
                found["skipped"] = False
                found["startedAt"] = None
                found["completedAt"] = None
                found["notes"] = merge_note(found.get("notes"), note)
        ...
        result.update(
            {
                ...
                "passes": found.get("passes") is True,
                "attempts": parse_attempts(found.get("attempts")),
                "skipped": skipped,
                "note": found.get("notes", ""),
            }
        )
...
verify_story_report() {
  local story_id="$1"
  local result_out="$2"
  python3 - "$PRD_PATH" "$story_id" "$ROOT_DIR" "$result_out" <<'PY'
import json
import re
import sys
from pathlib import Path
...
if not prd_path.exists():
    result["error"] = f"PRD not found: {prd_path}"
    result_out.write_text(json.dumps(result, indent=2) + "\n")
    sys.exit(0)
...
try:
    ...
    result["error"] = f"Invalid PRD JSON: {exc}"
    result_out.write_text(json.dumps(result, indent=2) + "\n")
    sys.exit(0)

stories = data.get("stories") if isinstance(data, dict) else None
if not isinstance(stories, list):
    result["error"] = "PRD stories are missing"
    result_out.write_text(json.dumps(result, indent=2) + "\n")
    sys.exit(0)
...
if not isinstance(story, dict):
    result["error"] = f"Story not found in PRD: {story_id}"
    result_out.write_text(json.dumps(result, indent=2) + "\n")
    sys.exit(0)
...
criteria = verification.get("passCriteria")
if not isinstance(criteria, list):
    criteria = story.get("passCriteria")
if not isinstance(criteria, list):
...
for artifact in artifacts:
    if "/verify/" in artifact and artifact.endswith(".md"):
        ...
        break
if not verify_rel:
    # Default verify path for this project
    verify_rel = f".codex/ralph-gitcontext/verify/{story_id}.md"

verify_path = Path(verify_rel)
if not verify_path.is_absolute():
    verify_path = root_dir / verify_path
result["verify_path"] = str(verify_path)

if not verify_path.exists():
    result["error"] = f"Verification report not found: {verify_path}"
    result_out.write_text(json.dumps(result, indent=2) + "\n")
    sys.exit(0)

text = verify_path.read_text(encoding="utf-8", errors="replace")
verified_line = re.search(r"(?mi)^VERIFIED:\s*YES\s*$", text) is not None
missing = [line for line in criteria if line not in text]

result["verified_line"] = verified_line
result["missing_criteria"] = missing
result["ok"] = verified_line and not missing
if not verified_line:
    result["error"] = "Verification report missing required line: VERIFIED: YES"
elif missing:
    result["error"] = "Verification report is missing one or more passCriteria lines"
...
for i in $(seq 1 "$MAX_ITERATIONS"); do
  ...
  if [ "$MODE" = "build" ]; then
    ...
    SELECT_OK="$(json_field "$STORY_META" "ok")"
    if [ "$SELECT_OK" != "true" ]; then
      SELECT_ERROR="$(json_field "$STORY_META" "error")"
      echo "Story selection failed: $SELECT_ERROR"
      exit 1
    fi
  ...
  if [ "$MODE" = "build" ]; then
    OUTCOME_META="$TMP_DIR/outcome-$RUN_TAG-$i.json"
    if [ "$CMD_STATUS" -ne 0 ]; then
      STATUS_LABEL="error"
      HAS_ERROR="true"
      REASON="Agent execution failed with status $CMD_STATUS; see $LOG_FILE"
      log_error "ITERATION $i $REASON"
      finalize_story "$STORY_ID" "fail" "$REASON" "$OUTCOME_META"
      echo "Iteration failed; story re-queued (or skipped by circuit breaker)."
    else
      if ! grep -q "<promise>COMPLETE</promise>" "$LOG_FILE"; then
        ...
        log_error "ITERATION $i $REASON"
        finalize_story "$STORY_ID" "fail" "$REASON" "$OUTCOME_META"
        echo "No completion signal; story re-queued (or skipped by circuit breaker)."
      else
        VERIFY_META="$TMP_DIR/verify-$RUN_TAG-$i.json"
        verify_story_report "$STORY_ID" "$VERIFY_META"
        VERIFY_OK="$(json_field "$VERIFY_META" "ok")"
        if [ "$VERIFY_OK" = "true" ]; then
          finalize_story "$STORY_ID" "pass" "Completion + verification gate passed." "$OUTCOME_META"
          echo "Completion signal + verification gate passed; story marked passes=true and done."
        else
          STATUS_LABEL="error"
          HAS_ERROR="true"
          VERIFY_ERROR="$(json_field "$VERIFY_META" "error")"
          VERIFY_PATH="$(json_field "$VERIFY_META" "verify_path")"
          REASON="Verification gate failed (${VERIFY_ERROR}). Verify report: ${VERIFY_PATH}"
          log_error "ITERATION $i $REASON"
          finalize_story "$STORY_ID" "fail" "$REASON" "$OUTCOME_META"
          echo "Verification gate failed; story re-queued (or skipped by circuit breaker)."
        fi
      fi
    fi
  else
